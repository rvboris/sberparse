...
describe("extractors", () => {
  it("parses payment account (SBER_DEBIT_2604)", async () => {
    const text = await fs.readFile(path.join(fixturesDir, "payment-2604.txt"), "utf-8");
    const extractor = new SBER_DEBIT_2604(text);
    ...
    expect(entries[0].authorisation_code).toBe("123456");
  ...
  it("throws when balance does not match", async () => {
    const text = await fs.readFile(path.join(fixturesDir, "payment-2604-bad-balance.txt"), "utf-8");
    const extractor = new SBER_DEBIT_2604(text);
    const entries = extractor.getEntries();
  ...
  it("throws when balance does not match for SBER_DEBIT_2603", async () => {
